...
session_start();
$aid=$_GET['article'];
$dbc=mysqli_connect("localhost","root","","test") or die (mysql_error());
...
function likedisabled()
{
 ...
 function calculatearticleimage($id)
    ...
    $url=calculatearticleimage($aid);
     function calculatearticleimage($aid)
    {
        //$imagearray=getimagesize("upload_pic/thumbnail_{$_SESSION['login_user']}.jpg");
        if(file_existsf("Articleimage/{$id}.jpg"))
        {
         return "Articleimage/{$id}.jpg";
        }
        else
...
<html lang="en" class="no-js">
	...
	<body>
		<div class="container">
			...
			<p> <a href="index.html" id="butt-on" class="glyphicon glyphicon-thumbs-up"> Like</a> <a href="index.html" id="butt-on1" class="glyphicon glyphicon-briefcase"> Buy</a></p>
			...
			<div id="display"  style="padding-top: 10px;background-color:#aaa; padding-left:10px;">
				...
				<button id="viewall">view all</button>
					...
					<?php
						...
						$query='select * from comments where articleid="'.$_GET['id'].'" limit 0,5';
						$result = mysqli_query($dbc,$query) or die('no query');
